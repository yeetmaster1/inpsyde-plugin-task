...
## End To End test
As i said i did an ete test to make up for my faultiy phpunit module but this test needs [installation](https://www.protractortest.org/#/tutorial).
After that installation you need to go to etoe_test folder and then to ETOE_sample.js file and change the wordpress password and username and the link of the admin page, I use xampp for the server so that address is in regards to my local host don't worry there are comments to direct you.
The reason you need to do the previous step is that the test runs the browser and goes to the address and since it's an admin menu page you need the password and username and the right link.
